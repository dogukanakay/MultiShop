...
namespace MultiShop.Catalog.Services.ContactServices
{
    public class ContactService : IContactService
    {
        ...
        {
            await _contactCollection.DeleteOneAsync(f => f.ContactId == id);
        }

        public async Task<List<ResultContactDto>> GetAllContactsAsync()
        {
            var values = await _contactCollection.Find(f => true).ToListAsync();
            return _mapper.Map<List<ResultContactDto>>(values);
        }

        public async Task<GetByIdContactDto> GetByIdContactAsync(string id)
        {
            var values = await _contactCollection.Find(f => f.ContactId == id).FirstOrDefaultAsync();
        ...
        public async Task UpdateContactAsync(UpdateContactDto updateContactDto)
        {
            ...
            await _contactCollection.FindOneAndReplaceAsync(f => f.ContactId == updateContactDto.ContactId, values);
        }
    }
}
