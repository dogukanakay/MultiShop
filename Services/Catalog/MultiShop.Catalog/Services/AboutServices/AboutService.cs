...
namespace MultiShop.Catalog.Services.AboutServices
{
    public class AboutService : IAboutService
    {
        ...
        {
            await _aboutCollection.DeleteOneAsync(f => f.AboutId == id);
        }

        public async Task<List<ResultAboutDto>> GetAllAboutsAsync()
        {
            var values = await _aboutCollection.Find(f => true).ToListAsync();
            return _mapper.Map<List<ResultAboutDto>>(values);
        }

        public async Task<GetByIdAboutDto> GetByIdAboutAsync(string id)
        {
            var values = await _aboutCollection.Find(f => f.AboutId == id).FirstOrDefaultAsync();
        ...
        public async Task UpdateAboutAsync(UpdateAboutDto updateAboutDto)
        {
            ...
            await _aboutCollection.FindOneAndReplaceAsync(f => f.AboutId == updateAboutDto.AboutId, values);
        }
    }
}
