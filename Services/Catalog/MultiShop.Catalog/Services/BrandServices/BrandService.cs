...
namespace MultiShop.Catalog.Services.BrandServices
{
    public class BrandService : IBrandService
    {
        ...
        {
            await _brandCollection.DeleteOneAsync(f => f.BrandId == id);
        }

        public async Task<List<ResultBrandDto>> GetAllBrandsAsync()
        {
            var values = await _brandCollection.Find(f => true).ToListAsync();
            return _mapper.Map<List<ResultBrandDto>>(values);
        }

        public async Task<GetByIdBrandDto> GetByIdBrandAsync(string id)
        {
            var values = await _brandCollection.Find(f => f.BrandId == id).FirstOrDefaultAsync();
        ...
        public async Task UpdateBrandAsync(UpdateBrandDto updateBrandDto)
        {
            ...
            await _brandCollection.FindOneAndReplaceAsync(f => f.BrandId == updateBrandDto.BrandId, values);
        }
    }
}
